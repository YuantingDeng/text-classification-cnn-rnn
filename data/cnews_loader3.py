...
def create_file(filename,train_dir,test_dir,val_dir,train_per_class_size,val_per_class_size):
    ...
    for i in range(len(data)):
        for j in range(len(category)):
            if data[i][0]==category[j] and train_class_num[j]<train_per_class_size:
                ftrain.write(data[i][0]+'\t'+data[i][1]+'\n')
                train_class_num[j]=train_class_num[j]+1
            elif data[i][0]==category[j] and val_class_num[j]<val_per_class_size:
                fval.write(data[i][0]+'\t'+data[i][1]+'\n')
                val_class_num[j]=val_class_num[j]+1
            else:
                ftest.write(data[i][0]+'\t'+data[i][1]+'\n')
